...
graph TD
    ...
    LB -->|1st Request| S1[Server 1]
    LB -->|2nd Request| S2[Server 2]
    LB -->|3rd Request| S3[Server 3]
    LB -.->|4th Request| S1
```
...
graph TD
    Client --> LB[Load Balancer]
    LB -->|5 Requests| S1[Server 1 - Weight 5]
    LB -->|1 Request| S2[Server 2 - Weight 1]
    LB -->|1 Request| S3[Server 3 - Weight 1]
```
...
graph TD
    Client --> LB[Load Balancer]
    LB -->|Checks connections| S1[Server 1: 10 active]
    LB -->|Routes here!| S2[Server 2: 2 active]
    LB -->|Checks connections| S3[Server 3: 5 active]
```
...
```mermaid
graph TD
    Client1[IP: 192.168.1.1] --> LB[Load Balancer]
    Client2[IP: 10.0.0.1] --> LB
    LB -->|Hash % N| S1[Server 1]
    LB -->|Hash % N| S2[Server 2]
```
