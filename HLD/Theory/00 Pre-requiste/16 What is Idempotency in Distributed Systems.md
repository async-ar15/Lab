...
Some operations are naturally idempotent. No matter how many times you run this, the result remains the same.
...
## 3. Strategies to Implement Idempotency
...
sequenceDiagram
    ...
    Server-->>Client: Timeout / Network Error (But Server Processed it)
    Client->>Server: Retry Request (ID: 123)
    Server->>Server: Check ID 123 -> Already Processed
    Server-->>Client: Success (No duplicate charge)
```
...
VALUES (1, 50) 
ON CONFLICT (item_id) 
DO UPDATE SET stock = inventory.stock + 50;
```
