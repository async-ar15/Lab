...
```mermaid
graph TD
    DB[(Large Database)] -->|Split| S1[(Shard 1)]
    DB -->|Split| S2[(Shard 2)]
    DB -->|Split| S3[(Shard 3)]
```
...
```mermaid
graph TD
    Data[Data: user_id] --> Hash(Hash Function)
    Hash -->|Hash % 2 == 0| Shard0[(Shard 0)]
    Hash -->|Hash % 2 == 1| Shard1[(Shard 1)]
```
...
graph TD
    ID[Data ID] --> Router
    Router -->|1 to 10000| Shard1[(Shard 1)]
    Router -->|10001 to 20000| Shard2[(Shard 2)]
```
...
```mermaid
graph TD
    Loc[User Location] --> Router
    Router -->|North America| Shard1[(Shard 1)]
    Router -->|Europe| Shard2[(Shard 2)]
    Router -->|Asia| Shard3[(Shard 3)]
```
...
```mermaid
graph TD
    Key[Shard Key] --> Lookup[Lookup Table]
    Lookup -->|Map Key A| ShardA[(Shard A)]
    Lookup -->|Map Key B| ShardB[(Shard B)]
```
