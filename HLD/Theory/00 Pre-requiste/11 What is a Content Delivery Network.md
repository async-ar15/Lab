...
sequenceDiagram
    participant User
    participant DNS
    participant EdgeServer as Edge Server (CDN)
    participant OriginServer as Origin Server
    
    User->>DNS: Request content (e.g., logo.png)
    DNS-->>User: Returns nearest Edge Server IP
    User->>EdgeServer: Request content (logo.png)
    alt Content is Cached
        EdgeServer-->>User: Serve cached content
    else Content NOT Cached
        EdgeServer->>OriginServer: Fetch content (logo.png)
