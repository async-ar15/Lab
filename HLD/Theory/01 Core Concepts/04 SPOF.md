...
Single Point of Failure
One critical node takes the whole system down. A replica keeps it running.

Single Instance
Redundant

UsersRoutingApp Tier
downtime 0s
dropped 0
System UP
Clients
...
serving 100%
100%
One app server sits behind the load balancer. Press Play.
...
A critical path depends on it.
There is no working alternative when it fails.
The failure causes unacceptable impact.
...
Fallback:
trending items / cached results

Limited but usable page
...
Remove it now.
Mitigate it with graceful degradation.
Monitor it and accept the risk.
Put it on the roadmap with a clear trigger.
...
SPOFs are dependency problems. Look at user flows, not just servers.
Redundancy only helps when failure modes are independent. Shared zones, accounts, networks, and control planes can defeat redundancy.
Caches and optional services can still cause cascading failures. A fallback path must have enough capacity.
Replication is not backup. You need both for critical data.
Failover must be tested. Untested standby systems often fail during the first real incident.
Graceful degradation reduces blast radius. Not every dependency should take down the whole product.
Operational SPOFs matter. Runbooks, permissions, deployment pipelines, and human knowledge can be single points too.
Accepting a SPOF can be valid. Make it a conscious trade-off based on risk, cost, and user impact.
