...
sequenceDiagram
    ...
    Client->>API: 1. Makes request
    API->>Database: 2. Interacts with databases or services
    Database-->>API: 
    API-->>Client: 3. Sends structured response (JSON/XML)
```
...
```mermaid
graph LR
    User[User] -->|Interacts visually| Frontend[Frontend (GUI)]
    Frontend -->|Sends API requests| Backend[Backend (APIs)]
    Backend -->|Processes data| DB[(Database)]
    DB --> Backend
    Backend -->|Sends responses| Frontend
```
...
2. Go to the API keys section and generate a key.
...
2. Click "New Request", enter the API endpoint URL (`https://api.openweathermap.org/data/3.0/weather?q=London&appid=YOUR_API_KEY`).
...
1. Fetch live weather data from the API.
2. Parse and extract relevant details (temperature, humidity, condition).
3. Display the weather report in a user-friendly format.